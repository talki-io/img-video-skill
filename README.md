# VisionBridge · 素材生成 Skill

> 一个 **Claude Agent Skill**：让 Claude / Claude Code 也能"生成图片和视频"。

Claude 本身不会生成像素。**VisionBridge 是一个即插即用的 Skill**——把它放进 Claude 的技能目录后，
...
Claude 会读取其中的**描述**来判断"这个任务要不要用这个技能"。VisionBridge 的核心就是
...
把本仓库克隆到 Claude 的技能目录，文件夹名用技能名 `visionbridge`：

```bash
# 个人级（对所有项目生效）
git clone https://github.com/talki-io/visionbridge-skill.git ~/.claude/skills/visionbridge

# 或 项目级（只对某个项目生效）
git clone https://github.com/talki-io/visionbridge-skill.git <你的项目>/.claude/skills/visionbridge
```

然后装依赖、填 API Key：

```bash
cd ~/.claude/skills/visionbridge
...
```
visionbridge/                # ← 放进 ~/.claude/skills/ 的技能文件夹
